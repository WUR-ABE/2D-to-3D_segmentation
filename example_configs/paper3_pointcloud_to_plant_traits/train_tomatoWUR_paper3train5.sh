...
ann_versions=0-paper-2Dto3D
dataset_folder=TomatoWUR/data/TomatoWUR/ann_versions/${ann_versions}/json/


CONFIG=semseg-pt-v3m1-0-base
CONFIG=semseg-swin3d-v1m1-0-small
config_version=_train5
CONFIG_DIR=example_configs/paper3/${CONFIG}${config_version}.py
DATE=`date +%Y%m%d_%H%M`
EXP_NAME=${DATE}${config_version}

EXP_DIR=TomatoWUR/data/TomatoWUR_experiments/models/${CONFIG}/${EXP_NAME}



if echo "$CONFIG" | grep -q "swin3d"; then
  WEIGHT="example_configs/paper3/Swin3D_RGBN_S_updated.pth"
else
  WEIGHT="example_configs/paper3/scannet-semseg-pt-v3m1-0-base_model_best_updated.pth"
fi
